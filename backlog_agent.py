...
def calcular_pascoa(ano: int) -> datetime.date:
    """Algoritmo de Meeus/Jones/Butcher para data da Páscoa."""
    a = ano % 19
    b = ano // 100; c = ano % 100; d = b // 4; e = b % 4
    f = (b + 8) // 25; g = (b - f + 1) // 3
    h = (19*a + b - d - g + 15) % 30
    i = c // 4; k = c % 4
    l = (32 + 2*e + 2*i - h - k) % 7
...
def feriados_nacionais_brasil(ano: int) -> set:
    """Retorna set de datas de feriados nacionais brasileiros."""
    fer = set()
    fixos = [(1,1),(4,21),(5,1),(9,7),(10,12),(11,2),(11,15),(12,25)]
    for mes, dia in fixos:
        try: fer.add(datetime.date(ano, mes, dia))
        except ValueError: pass
    pascoa = calcular_pascoa(ano)
    fer.add(pascoa - datetime.timedelta(days=48))  # Segunda Carnaval
    fer.add(pascoa - datetime.timedelta(days=47))  # Terça Carnaval
    fer.add(pascoa - datetime.timedelta(days=2))   # Sexta-feira Santa
    fer.add(pascoa)                                # Páscoa
    fer.add(pascoa + datetime.timedelta(days=60))  # Corpus Christi
    return fer

def feriados_periodo(d1: datetime.date, d2: datetime.date) -> set:
    """Feriados nacionais em um intervalo de datas (pode cruzar anos)."""
    fer = set()
    for ano in range(d1.year, d2.year + 1):
        fer.update(feriados_nacionais_brasil(ano))
    return fer

def proximo_dia_util(data: datetime.date, feriados: set) -> datetime.date:
    """Avança até o próximo dia útil (seg–sex, não feriado)."""
    while data.weekday() >= 5 or data in feriados:
        data += datetime.timedelta(days=1)
    return data

def ultimo_dia_util_ate(data: datetime.date, feriados: set) -> datetime.date:
    """Recua até o último dia útil até 'data' (inclusive)."""
    while data.weekday() >= 5 or data in feriados:
        data -= datetime.timedelta(days=1)
    return data
...
def distribuir_sprints(
    user_stories: list, horas_por_sprint: int, horas_por_semana: int,
    horas_deploy_sprint: int, horas_teste_aceitacao: int, horas_homologacao: int,
    data_inicio_projeto: datetime.date, semanas_sprint: int = 1
) -> list:
    """
    Distribui as US em sprints respeitando dependências e capacidade.
    Calcula datas reais de início/fim de cada sprint, descontando feriados.
    Retorna lista de sprints com US alocadas.
    """
    # Feriados do período estimado (projeto + 2 anos)
    todos_feriados = feriados_periodo(data_inicio_projeto,
                                      data_inicio_projeto + datetime.timedelta(days=730))

    us_concluidas = set()
    sprints = []
    sprint_num = 1
    semana = 1
    us_pendentes = list(user_stories)
    data_sprint_atual = proximo_dia_util(data_inicio_projeto, todos_feriados)

    while us_pendentes:
        sprint_cards = []
        horas_usadas = 0
        LIMITE = horas_por_sprint
        sprint_ids = set()  # IDs das US já alocadas nesta sprint

        # Preenche a sprint iterativamente, resolvendo dependências dentro da mesma sprint.
        # A cada US adicionada, re-verifica pendentes: se alguma dependia apenas de US
        # que já estão nesta sprint, ela também pode entrar (sem esperar a próxima sprint).
        muda = True
        while muda and (not sprint_cards or horas_usadas < LIMITE):
            muda = False
            for us in list(us_pendentes):
                deps_ids = []
                ...
                # Disponível se todas as deps estão concluídas em sprints anteriores
                # OU já foram alocadas nesta mesma sprint
                if all(d in us_concluidas or d in sprint_ids for d in deps_ids):
                    h = us.get("horas_estimadas", 3)
                    if not sprint_cards:  # Primeira US: sempre adiciona para garantir progresso
                        sprint_cards.append(us); horas_usadas += h
                        sprint_ids.add(us["id"]); us_pendentes.remove(us)
                        muda = True
                        if horas_usadas >= LIMITE: break
        ...
        # Fallback: dependência circular ou estado inconsistente
        if not sprint_cards:
            aviso(f"Dependência não resolvida detectada. Alocando {us_pendentes[0]['id']} mesmo assim.")
            us = us_pendentes.pop(0)
            sprint_cards = [us]; horas_usadas = us.get("horas_estimadas", 3)
            sprint_ids = {us["id"]}

        # semanas_estimadas: apenas para o label descritivo (de esforço em horas)
        semanas_estimadas = max(1, (horas_usadas + horas_por_semana - 1) // horas_por_semana)
        semana_str = f"Sem. {semana}" if semanas_sprint == 1 else f"Sem. {semana}–{semana + semanas_sprint - 1}"

        # Datas reais da sprint — sempre blocos fixos de semanas_sprint semanas
        data_inicio_sprint = data_sprint_atual
        data_fim_raw = data_inicio_sprint + datetime.timedelta(weeks=semanas_sprint) - datetime.timedelta(days=1)
        data_fim_sprint = ultimo_dia_util_ate(data_fim_raw, todos_feriados)
        data_sprint_atual = proximo_dia_util(data_fim_sprint + datetime.timedelta(days=1), todos_feriados)
        ...
        rfs = []
        for us in sprint_cards: rfs.extend(us.get("rf_ids", []))
        ...
        sprints.append({
            ...
            "data_fim": data_fim_sprint,
            "cards": sprint_cards,
            "tot_dev": tot_dev,
            "tot_teste": tot_teste,
            "horas_deploy": horas_deploy_sprint,
            "horas_teste_aceitacao": horas_teste_aceitacao,
            "horas_homologacao": horas_homologacao,
            "tot_horas_us": horas_usadas,
            "horas_estimadas": horas_usadas + horas_deploy_sprint + horas_teste_aceitacao + horas_homologacao,
            "entrega": entrega,
            "rfs": rfs_str,
        })

        for us in sprint_cards: us_concluidas.add(us["id"])
        sprint_num += 1
        semana += semanas_sprint
...
def gerar_docx(projeto: dict, requisitos: dict, user_stories: list, sprints: list, caminho: str):
    ...
    for sprint in sprints:
        dt_ini = sprint['data_inicio'].strftime('%d/%m/%Y') if sprint.get('data_inicio') else sprint['semana']
        dt_fim = sprint['data_fim'].strftime('%d/%m/%Y') if sprint.get('data_fim') else ''
        periodo = f"  ({dt_ini} – {dt_fim})" if dt_fim else ""
        add_heading(doc, f"Sprint {sprint['numero']} — {sprint['semana']}{periodo}", level=2, color="2E75B6")

        info_lines = [
            f"Objetivo: {sprint['entrega']}",
            f"Período: {dt_ini} – {dt_fim}  |  {sprint['semana']}",
            f"Dev: {sprint.get('tot_dev', 0)}h  |  Teste Dev: {sprint.get('tot_teste', 0)}h  |  Teste Aceitação: {sprint.get('horas_teste_aceitacao', 0)}h  |  Homologação: {sprint.get('horas_homologacao', 0)}h  |  Deploy: {sprint.get('horas_deploy', 0)}h  |  Total: {sprint['horas_estimadas']}h",
...
def gerar_xlsx(requisitos: dict, user_stories: list, sprints: list, configs: dict, caminho: str):
    ...
    total_horas_fixo = len(sprints) * (configs["horas_deploy_sprint"] + configs.get("horas_teste_aceitacao", 0) + configs.get("horas_homologacao", 0))
    ...
    COLS2=["Sprint","Data Início","Data Fim","Card(s)","Entrega Principal",
           "Horas Dev","Horas Teste","Teste Aceitação","Homologação","Horas Deploy","Total Horas","RFs","Status"]
    WIDS2=[10,13,13,18,38,10,10,14,13,11,11,22,20]
    ...
    for sp_idx, sprint in enumerate(sprints, 3):
        ...
        dt_ini = sprint["data_inicio"].strftime("%d/%m/%Y") if sprint.get("data_inicio") else ""
        dt_fim = sprint["data_fim"].strftime("%d/%m/%Y") if sprint.get("data_fim") else ""
        vals=[f"Sprint {sprint['numero']:02d}", dt_ini, dt_fim, cards_str,
              sprint["entrega"], sprint.get("tot_dev",0), sprint.get("tot_teste",0),
              sprint.get("horas_teste_aceitacao",0), sprint.get("horas_homologacao",0),
              sprint.get("horas_deploy",0), sprint["horas_estimadas"], sprint["rfs"]]
        for col_i,val in enumerate(vals,1):
            cell=ws2.cell(row=sp_idx,column=col_i,value=val)
            cell.fill=fill_xl(bg); cell.border=brd_xl()
            cell.alignment=aln_xl(h="center" if col_i!=5 else "left",v="center",wrap=True)
            cell.font=fnt_xl(size=9,bold=(col_i==1))

        # Status automático baseado na aba Backlog
        ...
        sc=ws2.cell(row=sp_idx,column=13,value=f_status)
        sc.font=fnt_xl(size=9,color="555555"); sc.fill=fill_xl("F5F5F5")
        sc.alignment=aln_xl(h="center",v="center"); sc.border=brd_xl()
    ...
    # Somas: F=Dev, G=Teste, H=Teste Aceitação, I=Homologação, J=Deploy, K=Total
    for col_i, val in enumerate([
        f"=SUM(F3:F{tr-1})", f"=SUM(G3:G{tr-1})", f"=SUM(H3:H{tr-1})",
        f"=SUM(I3:I{tr-1})", f"=SUM(J3:J{tr-1})", f"=SUM(K3:K{tr-1})",
        "Total Final", ""
    ], 6):
...
def main():
    ...
    while True:
        try:
            data_str_ini = pergunta("Data de início do projeto (DD/MM/AAAA, ex: 05/05/2026):").strip()
            data_inicio_projeto = datetime.datetime.strptime(data_str_ini, "%d/%m/%Y").date()
            fer_ini = feriados_nacionais_brasil(data_inicio_projeto.year)
            data_inicio_projeto = proximo_dia_util(data_inicio_projeto, fer_ini)
            ok(f"Início confirmado: {data_inicio_projeto.strftime('%d/%m/%Y')} (primeiro dia útil)")
            break
        except ValueError:
            aviso("Data inválida. Use o formato DD/MM/AAAA (ex: 05/05/2026).")
    ...
    info("\nTeste de Aceitação e Homologação são medidos em DIAS ÚTEIS (1 dia = 8h).")
    info("Essas etapas não dependem das horas produtivas do dev.\n")

    dias_teste_aceitacao = ask_int("Dias úteis para Teste de Aceitação por sprint (ex: 1 ou 2):")
    if dias_teste_aceitacao < 0: dias_teste_aceitacao = 0
    horas_teste_aceitacao = dias_teste_aceitacao * 8

    dias_homologacao = ask_int("Dias úteis para Homologação do Cliente por sprint (ex: 1 ou 2):")
    ...
    sprints = distribuir_sprints(
        user_stories, horas_disp_us, horas_por_semana, horas_deploy_sprint,
        horas_teste_aceitacao, horas_homologacao, data_inicio_projeto,
    ...
    configs = {
        ...
        "dias_teste_aceitacao": dias_teste_aceitacao,
        "horas_teste_aceitacao": horas_teste_aceitacao,
